...
// 计算某分类在指定范围内的已用预算（支出为负、收入为正，最后取净支出的正值）
// year: 数字年份
// monthOrNullForYearView: 1-12 表示某个月；null 表示整年
function calcBudgetUsedForCategory(cat, year, monthOrNullForYearView) {
  ...
  // “已用额度”按之前约定：取净支出的正值（收入可抵扣支出）
  // return sum < 0 ? -sum : 0;
  return sum;
}
function calcBudgetUsedForCategoryOut(cat, year, monthOrNullForYearView) {
  if (!window.state) return 0;
  const state = window.state;
  const isYearView = (monthOrNullForYearView == null);
...
function calcBudgetUsedForCategoryIn(cat, year, monthOrNullForYearView) {
  if (!window.state) return 0;
  const state = window.state;
  const isYearView = (monthOrNullForYearView == null);
...
// ====== 预算渲染（基于原来的 renderBudget(targetMonth)，扩展支持按年，同时保留原有样式和进度条） ======
// viewMode: 'month' | 'year'
//   - 'month'：target 表示 'YYYY-MM'（或 null -> 当前月）
//   - 'year' ：target 表示 'YYYY'    （或 null -> 当前年）
function renderBudget(target = null, viewMode = 'month') {
  ...
  if (viewMode === 'year') {
    ...
    // 按月：保持与原 renderBudget(targetMonth) 一致
    let ymStr = target;
    if (!ymStr) {
  ...
  // 顶部说明文字（原先就有的 budgetMonthLabel）
  const labelEl = $('#budgetMonthLabel');
  if (labelEl) {
    if (viewMode === 'year') {
      labelEl.textContent = `${year} 年 · 基准：${state.prefs.baseCurrency}`;
    } else {
      const mm = String(month).padStart(2, '0');
      labelEl.textContent = `${year}-${mm} · 基准：${state.prefs.baseCurrency}`;
    }
  }
  ...
  // 1. 确定要显示的所有分类：
  //    - 所有已有预算的分类（state.budgets 的 key）
  //    - 所有当前分类（state.categories）
  //    => 合并去重；没有预算的分类，预算视为 0，也要显示
  const catSet = new Set();
  if (state.categories && state.categories.length) {
    state.categories.forEach(c => catSet.add(c));
  }
  if (state.budgets) {
    Object.keys(state.budgets).forEach(c => catSet.add(c));
  }
  const cats = Array.from(catSet);

  if (cats.length === 0) {
    grid.innerHTML = '<p class="muted">当前没有可用分类，请先在“交易分类管理”中添加分类。</p>';
    return;
  }

  // 2. 遍历所有分类，渲染卡片（保留原渲染样式和进度条）
  cats.forEach(cat => {
    const monthBudget = Number(state.budgets?.[cat] || 0);
    // 按年时：期间预算 = 月预算 * 12；按月时：期间预算 = 月预算
    const periodBudget = viewMode === 'year' ? (monthBudget * 12) : monthBudget;

    // 使用新的 calcBudgetUsedForCategory：收入+支出净额→取净支出正值
    const usedOut = calcBudgetUsedForCategoryOut(cat, year, viewMode === 'year' ? null : month);
    const usedIn = calcBudgetUsedForCategoryIn(cat, year, viewMode === 'year' ? null : month);

    if (usedOut != 0 || usedIn != 0) {

      let used = usedOut - usedIn;
      ...
      if (used < 0) {
        ...
        // 按原先思路：ratio = 已用 / 预算
        ratio = periodBudget > 0 ? (used / periodBudget) : 0;
      }

      if (!isFinite(ratio)) ratio = 0;
      if (ratio < 0) ratio = 0;

      const remain = periodBudget - used;
      let barClass = 'ok';
      if (ratio >= 1) {
        barClass = 'over';
      } else if (ratio >= 0.8) {
      ...
      card.className = 'card';
      card.innerHTML = `
        <div class="content budget-card" data-bdg-cat="${cat}">
          <div class="row">
            <h4>${cat}</h4>
            <div class="space"></div>
          ...
          <div class="row" style="margin-top:6px">
            <span class="muted">期间预算：</span>
            <strong class="amount">${fmtAmount(periodBudget, baseCur)}</strong>
          </div>
          <div class="row" style="margin-top:4px">
            <span class="muted">${laber}</span>
            <strong class="amount ${barClass}">${fmtAmount(used, baseCur)}</strong>
          </div>
          <div class='row' style='margin-top:6px'>
          <div class='progress ${barClass}' style='flex:1'>
          <i style='width:${pct}%'></i>
          </div>
  ...
  // 3. 事件绑定：整卡点击、明细、编辑、删除

  // 整卡点击 = 明细
  $$('#budgetGrid .budget-card').forEach(el => {
    const cat = el.dataset.bdgCat;
    el.onclick = (e) => {
      if (e.target.closest('button')) return; // 避免点击按钮也触发
      const periodStr = viewMode === 'year'
        ? String(year)
        : `${year}-${String(month).padStart(2, '0')}`;
      if (window.showBudgetDetail) window.showBudgetDetail(cat, periodStr, viewMode);
    };
  });
...
// 显示预算分类的交易明细
// period: 视图=按月时为 'YYYY-MM'，视图=按年时为 'YYYY'
function showBudgetDetail(category, period, viewMode = 'month') {
  if (!window.state) return;
  const state = window.state;

  const tbody = $('#budgetDetailTable tbody');
  const title = $('#budgetDetailTitle');
  const totalEl = $('#budgetDetailTotal');
  const card = $('#budgetDetailCard');
  if (!tbody || !title || !totalEl || !card) return;
  ...
  rows.forEach(r => {
    ...
    tr.innerHTML = `
      ...
      <td>${r.payee}</td>
      <td>${r.memo}</td>
      <td style="text-align:right" class="amount ${r.amount < 0 ? 'negative' : 'positive'}">
  ...
  const periodLabel = viewMode === 'year'
    ? `${year} 年`
    : `${year}-${String(month).padStart(2, '0')}`;
  title.textContent = `交易明细 · ${category} · ${periodLabel}`;
