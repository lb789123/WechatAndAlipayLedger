...
async function boot(){
  ...
  window.addEventListener('load', ()=>{
    const pid=document.querySelector('#dataProfileId'); if(pid && window.state) pid.textContent=window.state.profileId;
    const bx=document.querySelector('#btnExportData'); if(bx) bx.onclick=window.exportCurrentIdData;
    const bi=document.querySelector('#btnImportData'); if(bi) bi.onclick=()=>document.querySelector('#importFile')?.click();
    const fi=document.querySelector('#importFile'); if(fi) fi.onchange=(e)=>{ const f=e.target.files && e.target.files[0]; if(f && window.importCurrentIdDataFromFile) window.importCurrentIdDataFromFile(f); e.target.value=''; };
